...
function hsvColorName(h: number, s: number, v: number): string {
  ...
  if (h > 200 && h < 260 && v >= 25 && v < 60 && s > 25 && s < 70)
    return 'Denim';
...
export function detectSeason(yoloType?: string, type?: string): string {
  ...
  if (
    /outwear|coat$|parka|puffer|sweater|hoodie|jumper|pullover|cardigan/.test(t)
  )
    return 'Winter';
...
export const YOLO_MAX_DIM = 640;
const YOLO_JPEG_QUALITY = 0.5;

export function compressForYolo(blob: Blob): Promise<File> {
  return new Promise((resolve, reject) => {
    const img = new Image();
    ...
    img.onload = () => {
      URL.revokeObjectURL(url);
      let { width, height } = img;
      if (width <= YOLO_MAX_DIM && height <= YOLO_MAX_DIM) {
        resolve(new File([blob], 'yolo.jpg', { type: 'image/jpeg' }));
        return;
      }
      if (width > height) {
        height = Math.round(height * (YOLO_MAX_DIM / width));
        width = YOLO_MAX_DIM;
      } else {
        width = Math.round(width * (YOLO_MAX_DIM / height));
        height = YOLO_MAX_DIM;
      }
      const canvas = document.createElement('canvas');
      canvas.width = width;
      ...
      canvas.toBlob(
        (b) => {
          if (b) resolve(new File([b], 'yolo.jpg', { type: 'image/jpeg' }));
          else reject(new Error('YOLO compression failed'));
        },
        'image/jpeg',
        YOLO_JPEG_QUALITY,
      );
...
export const DETECT_MAX_DIM = 900;
const DETECT_JPEG_QUALITY = 0.6;

export function compressForGemini(blob: Blob): Promise<File> {
  return new Promise((resolve, reject) => {
    const img = new Image();
    const url = URL.createObjectURL(blob);
    img.onload = () => {
      URL.revokeObjectURL(url);
      let { width, height } = img;
      if (width <= DETECT_MAX_DIM && height <= DETECT_MAX_DIM) {
        resolve(new File([blob], 'gemini.jpg', { type: 'image/jpeg' }));
        return;
      }
      if (width > height) {
        height = Math.round(height * (DETECT_MAX_DIM / width));
        width = DETECT_MAX_DIM;
      } else {
        width = Math.round(width * (DETECT_MAX_DIM / height));
      ...
      canvas.toBlob(
        (b) => {
          if (b) resolve(new File([b], 'gemini.jpg', { type: 'image/jpeg' }));
          else reject(new Error('Gemini compression failed'));
        },
        'image/jpeg',
        DETECT_JPEG_QUALITY,
      );
...
export function scaleDetections(
  ...
  return items
    .map((i) => ({
      ...
      box: [
        Math.round(i.box[0] * scaleX),
        Math.round(i.box[1] * scaleY),
        Math.round(i.box[2] * scaleX),
        Math.round(i.box[3] * scaleY),
...
function mergeAbortSignals(a: AbortSignal, b: AbortSignal): AbortSignal {
  ...
  const onAbort = () =>
    controller.abort(
      a.aborted
        ? a.reason
        : b.reason || new DOMException('Aborted', 'AbortError'),
    );
