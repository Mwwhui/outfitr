...
import { useLocationZones, locationZonesOptions, useClosetLayout, closetLayoutOptions } from '@/hooks/queries/locations';
import { useToggleFavorite } from '@/hooks/mutations/clothing';
import Image from 'next/image';
import Loader from '../components/Loader';
import WardrobeFilters from '../components/WardrobeFilters';
import DuplicateCompareModal, { type DuplicateGroup } from '../components/DuplicateCompareModal';
import ClosetFloorplan from '../components/wardrobe/ClosetFloorplan';
import { WardrobeIcon } from '../components/wardrobe/ZoneIcons';

export default function WardrobePageInner() {
  ...
  const { data: clothes, isLoading: clothesLoading } = useClothes(
    session?.user?.id,
  );
  const { data: clusterData, isLoading: clustersLoading } = useClusters(
    session?.user?.id,
  );
  const { data: zonesData, isLoading: zonesLoading } = useLocationZones(
    session?.user?.id,
  );
  const { data: layoutData, isLoading: layoutLoading } = useClosetLayout(session?.user?.id);
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'closet'>('grid');
  ...
  const jumpToColors = useMemo(() => {
    ...
    return [
      ...new Set(selectedClusterData.groups.map((g) => g.color || 'Other')),
    ];
  }, [selectedClusterData]);
  ...
  return (
    <div className="min-h-screen">
      <div className="px-6 pt-8 pb-4 max-w-7xl mx-auto flex items-center justify-between">
        <div>
        ...
        <div className="flex items-center bg-surface-container-low rounded-xl p-1 border border-outline-variant">
          ...
          >
            <WardrobeIcon width={16} height={16} />
            Closet
          </button>
        </div>
      </div>
      ...
      <div className="px-6 pb-16 max-w-7xl mx-auto space-y-8">
        {viewMode === 'closet' ? (
          /* Closet 2D Floorplan View */
          <>
            {zonesLoading || layoutLoading ? (
              ...
              <ClosetFloorplan
                ...
                onRefetch={() => {
                  ...
                  queryClient.invalidateQueries(locationZonesOptions(session?.user?.id));
                  queryClient.invalidateQueries(closetLayoutOptions(session?.user?.id));
                }}
              />
            )}
          </>
            ...
            {selectedClusterData?.groups ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 mt-4">
                {filteredClothes.map((item) => (
                  <div
                    key={item.id}
                    className="bg-white rounded-xl shadow hover:shadow-lg cursor-pointer transition-transform transform hover:scale-105"
                    onClick={() => router.push(`/wardrobe/${item.id}`)}
