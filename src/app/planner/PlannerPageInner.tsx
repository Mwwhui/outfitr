...
import {
  hasCompatibleUseCases,
  hasCompatibleSeasons,
} from '@/lib/suggestOutfits';
...
export default function PlannerPageInner() {
  ...
  useEffect(() => {
    ...
    const found = outfitPlans.find((o) => o.time_slot === timeSlot);
  ...
  const handleDrop = (slot: OutfitSlotKey, clothingId: string) => {
    ...
    const filled = Object.entries(slots).filter(
      ([key, val]) => key !== slot && val !== null,
    ) as [OutfitSlotKey, ClothingItem][];
            ...
            <div className="fixed inset-0 z-50 flex">
              ...
              <div className="relative ml-auto w-full max-w-md bg-white shadow-xl h-full overflow-y-auto">
                ...
                <div className="sticky top-0 bg-white border-b border-slate-200 px-5 py-4 flex items-center justify-between z-10">
                  <h2 className="text-lg font-semibold text-[#0f172a] font-headline">
                    Outfit Suggestions
                  </h2>
                  <button
                    onClick={() => setShowSuggestions(false)}
                      ...
                      <span className="group relative text-sm font-medium text-slate-600 cursor-pointer">
                        ...
                        <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2 px-3 py-2 rounded-lg bg-slate-800 text-white text-xs shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20">
                          <p className="font-medium">
                            {panelWeather.description}
                          </p>
                          <p className="text-slate-300">
                            {Math.round(panelWeather.temperature)}°C
                            {panelWeather.feelsLike != null
                              ? ` · Feels ${Math.round(panelWeather.feelsLike)}°C`
                              : ''}
                          </p>
                          <p className="text-slate-300">
                            {panelWeather.humidity != null
                              ? `Humidity ${panelWeather.humidity}%`
                              : ''}
                            {panelWeather.humidity != null &&
                            panelWeather.windSpeed != null
                              ? ' · '
                              : ''}
                            {panelWeather.windSpeed != null
                              ? `Wind ${Math.round(panelWeather.windSpeed)} km/h`
                              : ''}
                          </p>
                        </div>
                      </span>
                ...
                {panelMode === 'complete' &&
                  [
                    ...
                    <div className="mx-5 mt-3 px-4 py-3 rounded-lg bg-blue-50 border border-blue-200 text-sm text-blue-800">
                      ...
                      item(s) locked in — suggestions will include your current
                      selections
                    </div>
                  )}
                ...
                <div className="px-5 py-4 space-y-4">
                  {panelLoading ? (
                    ...
                    <p className="text-slate-500 text-center py-16 text-sm">
                      No suggestions found — try a different occasion or add
