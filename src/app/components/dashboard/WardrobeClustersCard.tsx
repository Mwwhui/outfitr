...
function InsightRow({ cluster, onViewCluster }: { cluster: ClusterGroup; onViewCluster: (id: number) => void }) {
  const { insight, label, color } = cluster;

  const primaryStat = (() => {
    switch (label) {
      case 'Keep in Rotation':
        return { value: `${insight.avgWear}`, unit: 'avg wears', detail: `${Math.round(insight.wearVsAverage)}% vs avg` };
      case 'Save for Occasions':
        return { value: `$${insight.totalValue}`, unit: 'total value', detail: `$${insight.avgPrice} avg each` };
      case 'Ready to Let Go':
        return { value: `${100 - insight.wearVsAverage}%`, unit: 'less worn', detail: 'than your average item' };
      default:
        return { value: `${cluster.size}`, unit: 'items', detail: `$${insight.avgPrice} avg` };
    }
  })();
    ...
    <div className="flex items-start gap-3 py-2.5 first:pt-0 last:pb-0">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline justify-between gap-2">
          ...
          <span className="text-xs text-gray-400 whitespace-nowrap">{cluster.size} item{cluster.size > 1 ? 's' : ''}</span>
        </div>

        <div className="flex items-baseline gap-1.5 mt-0.5">
          <span className="text-lg font-bold text-[#0f172a]">{primaryStat.value}</span>
          <span className="text-[11px] text-gray-500">{primaryStat.unit}</span>
          <span className="text-[11px] text-gray-400">· {primaryStat.detail}</span>
        </div>

        <div className="flex flex-wrap gap-x-2 gap-y-0.5 mt-1">
          {insight.typeBreakdown.slice(0, 3).map((t) => (
            <span key={t.type} className="text-[11px] text-gray-400">
              {t.type} <span className="font-medium text-gray-500">{t.percentage}%</span>
            </span>
          ))}
        </div>
    ...
    return (
      <div className="bg-white rounded-3xl shadow-sm p-5">
        <p className="text-xs text-gray-500">
          ...
          <button onClick={onRefresh} className="underline text-gray-600 hover:text-gray-800">
            Retry
          </button>
        </p>
      </div>
    ...
    <div className="bg-white rounded-3xl shadow-sm p-5">
      ...
      <div className="divide-y divide-gray-50">
        {clusters.map((cluster) => (
          <InsightRow key={cluster.id} cluster={cluster} onViewCluster={onViewCluster} />
        ))}
      </div>
    </div>
  );
