...
export async function POST(req: Request) {
  try {
    ...
    const cached = SUGGEST_CACHE.get(user_id);
    ...
    if (!clothes || clothes.length < 3) {
      return NextResponse.json({ suggestions: [], message: "Need at least 3 items for suggestions" });
    }
    ...
    // Generate suggestions using existing engine (null weather = no season filter)
    const suggestions = suggestOutfits(clothes, null, "casual", pairFreq);

    // Filter out already-worn combos and take top 5
    const fresh = suggestions
      .filter((s) => {
        const ids = s.items.map((i) => i.id).sort().join("::");
        return !wornCombos.has(ids);
      })
      .slice(0, 5);

    if (fresh.length === 0) {
      return NextResponse.json({ suggestions: [], message: "All combinations have been worn! Try something new." });
    }

    // Send to Gemini for reasoning
    const comboDescriptions = fresh.map((s, i) => {
      const itemNames = s.items.map((it) => `${it.name} (${it.color || "no color"}, ${it.type})`);
...
[
  ...
  ...
    ...
    let aiReasoning: Array<{ index: number; reasoning: string; style: string }> = [];
    if (response.ok) {
    ...
    const result = fresh.map((s, i) => {
      ...
      return {
        ...
        score: s.score,
        ai_reasoning: ai?.reasoning || `${s.items.map((it) => it.name).join(" + ")} — scored ${s.score}/100`,
        style: ai?.style || "safe",
        color_harmony: s.breakdown?.find((b) => b.label === "Color Harmony")?.value || 0,
      };
    });

    // Cache the result (1-day TTL)
    const responseData = { suggestions: result };
    SUGGEST_CACHE.set(user_id, { data: responseData, ts: Date.now() });
